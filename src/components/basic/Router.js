...
import {mainContainer} from "@/config/antConfig"
      ...
      {routes.map((route, index) => {
        if (route.redirect) {
          return (
            <Route
              ...
              path={route.path}
              exact={route.exact}
              render={ ()=> (<Redirect from={PATH_PREFIX + route.path} to={PATH_PREFIX + route.redirect}/>) }
            />
          )
        }
        ...
        return (
          <Route
            ...
            strict={route.strict}
            render={(props) => {
              let renderChildRoutes = renderRoutes(route.children,{...singlespa});
              ...
              if (route.component) {
                ...
                if(isCheckAuth && route.check){
                  if(rootStore.checkAuth(route.id)){
                    ...
                    return (
                      <Redirect from={PATH_PREFIX + route.path} to={PATH_PREFIX + '/noauth'}/>
                    )
                  }
                }else {
                  return (
...
const Routers = ({...singlespa}) => {
	return <HashRouter><div id={mainContainer}>{renderRoutes(routes,{...singlespa})}</div></HashRouter>;
};
