...
;(function () {
  let _theme =
    localStorage.getItem('theme') || window.DATACENTERAPP.defaultTheme
  let bodyEl = document.querySelector('body')
  bodyEl.setAttribute('data-theme', _theme)
})()

const RootAevt = ({ ...singlespa }) => {
  console.log(singlespa, 'root-singlespa')
  ...
  // 自定义Provider
  const MyProvider =
    portalStore && Object.keys(portalStore).length
      ? ContextManager.Provider
      : Fragment
  const MyProps =
    portalStore && Object.keys(portalStore).length ? { value: portalStore } : {}
  return (
